...
pub fn init(config: c.HowlRenderSurfaceTextConfig) callconv(.c) c.HowlRenderSurfaceTextHandle {
    ...
    const owner = surface_text.SurfaceTextOwner.create(std.heap.c_allocator, .{
...
pub fn deinit(value: c.HowlRenderSurfaceTextHandle) callconv(.c) void {
    const owner = handle_owner.surfaceTextOwner(value) orelse return;
    owner.destroy();
}

pub fn isValidFont(value: c.HowlRenderSurfaceTextHandle) callconv(.c) c_int {
...
pub fn setFontSize(value: c.HowlRenderSurfaceTextHandle, font_size_px: u16) callconv(.c) c_int {
    const owner = handle_owner.surfaceTextOwner(value) orelse return c.HOWL_RENDER_CALL_MISSING_HANDLE;
...
pub fn setCursorBlinkVisible(value: c.HowlRenderSurfaceTextHandle, visible: u8) callconv(.c) c_int {
    const owner = handle_owner.surfaceTextOwner(value) orelse return c.HOWL_RENDER_CALL_MISSING_HANDLE;
