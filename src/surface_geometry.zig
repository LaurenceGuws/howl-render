...
) callconv(.c) c.HowlRenderFrameLayoutResult {
    const owner = handle_owner.surfaceTextOwner(value) orelse return .{ .status = c.HOWL_RENDER_CALL_MISSING_HANDLE, .cell_px = .{ .width = 0, .height = 0 }, .grid = .{ .cols = 0, .rows = 0 } };
    const layout = owner.session.deriveFrameLayout(
        owner.config,
        pixelIn(render_px),
        pixelIn(grid_px),
    ) catch {
...
pub fn syncGeometry(
    value: c.HowlRenderSurfaceTextHandle,
    geometry: c.HowlRenderGeometry,
) callconv(.c) c.HowlRenderGeometryResponse {
    const owner = handle_owner.surfaceTextOwner(value) orelse return .{ .status = c.HOWL_RENDER_CALL_MISSING_HANDLE, .changed = 0, .render_px = .{ .width = 0, .height = 0 }, .grid_px = .{ .width = 0, .height = 0 }, .cell_px = .{ .width = 0, .height = 0 }, .geometry_epoch = 0 };
    const layout = owner.session.deriveFrameLayout(
        owner.config,
