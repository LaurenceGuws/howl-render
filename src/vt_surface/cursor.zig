...
pub fn mapVtSurfaceCursor(source: vt_surface.VtSurface, theme: color.SurfaceTheme) CursorPresentation {
    std.debug.assert(source.cursor.row < source.rows);
    std.debug.assert(source.cursor.col < source.cols);
    const extra_cursors = mapVtSurfaceExtraCursors(source.extra_cursors[0..source.extra_cursor_count]);
    return .{
        .focused = source.cursor_focused,
        .visible = source.cursor.visible != 0,
        .blink = source.cursor.blink != 0,
        .shape = mapCursorShape(source.effective_shape),
        .beam_thickness = theme.cursor_beam_thickness,
        .underline_thickness = theme.cursor_underline_thickness,
        .cursor_opacity = source.cursor_opacity,
        .text_blink_opacity = source.text_blink_opacity,
        .cursor_color = if (source.cursor_color.kind == 0) mapCursorColor(theme.cursor_default_color) else mapCursorColor(source.cursor_color),
        .cursor_text_color = if (source.cursor_text_color.kind == 0) mapCursorColor(theme.cursor_text_color) else mapCursorColor(source.cursor_text_color),
...
pub fn mapStateCursor(state: anytype, theme: color.SurfaceTheme) CursorPresentation {
    ...
    const visible = boolOrByte(state.cursor.visible);
    const blink = if (@hasField(@TypeOf(state.cursor), "blink")) boolOrByte(state.cursor.blink) else false;
    return .{
        .focused = true,
        .visible = visible,
        .blink = blink,
        ...
        .beam_thickness = theme.cursor_beam_thickness,
        .underline_thickness = theme.cursor_underline_thickness,
        .cursor_opacity = if (visible) 255 else 0,
...
fn boolOrByte(value: anytype) bool {
    return if (@TypeOf(value) == bool) value else value != 0;
}

fn mapCursorShape(shape: u8) CursorShape {
    return switch (shape) {
...
fn mapLegacyStateCursorShape(shape: anytype) CursorShape {
    switch (@typeInfo(@TypeOf(shape))) {
        .int, .comptime_int => return mapCursorShape(@intCast(shape)),
        else => {},
    }
...
fn mapCursorColor(color_value: c.HowlVtColor) CursorColor {
...
fn mapVtSurfaceExtraCursors(source: []const c.HowlVtExtraCursor) [max_extra_cursors]ExtraCursorPresentation {
    ...
    for (source, 0..) |cursor_value, index| {
        extra_cursors[index] = .{
            ...
            .shape = @enumFromInt(cursor_value.shape),
            .mode = @enumFromInt(cursor_value.mode),
            .shape_follows_main = cursor_value.shape_follows_main != 0,
            .color_follows_main = cursor_value.color_follows_main != 0,
...
fn mapCursorTrailSource(source: []const c.HowlRenderHostCursorTrailRect) CursorTrailSource {
...
test "renderable content cursor presentation maps widened vt_surface truth" {
    ...
    const visible = mapVtSurfaceCursor(.{
        .cols = 1,
        .rows = 1,
        .history_count = 0,
        .scroll_row = 0,
        ...
        .cells = cells[0..],
        .cursor = .{ .visible = 1, .row = 0, .col = 0, .shape = c.HOWL_VT_CURSOR_SHAPE_BEAM, .blink = 1, .position_changed_by_client_at_ms = 7, .cell_cols = 2, .cell_rows = 3 },
        .cursor_color = .{ .kind = 2, .value = 0x010203 },
        .cursor_text_color = .{ .kind = 1, .value = 9 },
        .cursor_opacity = 120,
        .text_blink_opacity = 45,
        .cursor_focused = false,
        .effective_shape = c.HOWL_VT_CURSOR_SHAPE_BEAM,
        .extra_cursor_count = 1,
        .extra_cursors = [_]c.HowlVtExtraCursor{.{
            .row = 2,
            .col = 3,
            .rows = 4,
            .cols = 5,
            .shape = 3,
            .mode = 1,
            .shape_follows_main = 1,
            .color_follows_main = 0,
            .cursor_color = .{ .kind = 1, .value = 4 },
            .text_color = .{ .kind = 2, .value = 0x040506 },
        }} ++ [_]c.HowlVtExtraCursor{.{
            .row = 0,
            .col = 0,
            .rows = 0,
            .cols = 0,
            .shape = 0,
            .mode = 0,
            .shape_follows_main = 0,
            .color_follows_main = 0,
            .cursor_color = .{ .kind = 0, .value = 0 },
            .text_color = .{ .kind = 0, .value = 0 },
        }} ** (max_extra_cursors - 1),
        .cursor_trail_count = 1,
        .cursor_trail_rects = [_]c.HowlRenderHostCursorTrailRect{.{
            .row = 6,
            .col = 7,
            .rows = 8,
            .cols = 9,
        ...
        }} ++ [_]c.HowlRenderHostCursorTrailRect{.{
            .row = 0,
            .col = 0,
            .rows = 0,
            .cols = 0,
...
test "renderable content cursor presentation keeps explicit no-shape" {
    ...
    const visible = mapVtSurfaceCursor(.{
        ...
        .is_alternate_screen = false,
        .cells = cells[0..],
        .cursor = .{ .visible = 1, .row = 0, .col = 0, .shape = c.HOWL_VT_CURSOR_SHAPE_NONE, .blink = 1, .position_changed_by_client_at_ms = 0, .cell_cols = 1, .cell_rows = 1 },
        .cursor_color = .{ .kind = 0, .value = 0 },
        .cursor_text_color = .{ .kind = 0, .value = 0 },
        .effective_shape = c.HOWL_VT_CURSOR_SHAPE_NONE,
        .extra_cursor_count = 0,
        .extra_cursors = [_]c.HowlVtExtraCursor{.{}} ** max_extra_cursors,
        .cursor_trail_count = 0,
        .cursor_trail_rects = [_]c.HowlRenderHostCursorTrailRect{.{}} ** max_cursor_trail_rects,
        .colors = std.mem.zeroes(c.HowlVtRenderColorState),
        .selection = std.mem.zeroes(c.HowlVtSelection),
        .cursor_phase_visible = true,
        .dirty_rows = @constCast(&dirty_rows),
