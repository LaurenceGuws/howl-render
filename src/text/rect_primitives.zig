...
pub const RectDecorationLayout = struct {
    ...
    pub fn init(cell_layout: render.CellLayout, cell_grid: render.CellGrid) RectDecorationLayout {
        return .{
            .cell_grid = cell_grid,
            .geometry = decorationGeometryForCellLayout(cell_layout),
            .cols = @max(@as(u32, cell_grid.cols), 1),
            .cell_w_px = cell_layout.cell_w_px,
            .cell_h_px = cell_layout.cell_h_px,
        };
    }
};

pub fn rectDecorationLayout(cell_layout: render.CellLayout, cell_grid: render.CellGrid) RectDecorationLayout {
    return .init(cell_layout, cell_grid);
}

pub fn countClearDraws(cell_grid: render.CellGrid, damage: text_damage.NormalizedDamage) usize {
    if (damage.full) return 0;
    const rows = @min(cell_grid.rows, text_damage.damageRowCount(damage));
    var count: usize = 0;
    var row: u16 = 0;
    while (row < rows) : (row += 1) {
        if (text_damage.dirtyRowSpan(damage, cell_grid, row) == null) continue;
        count += 1;
    }
    return count;
}
...
pub fn countRectDecorationDraws(cells: []const render.RenderableCell, cell_layout: render.CellLayout, cell_grid: render.CellGrid, damage: text_damage.NormalizedDamage) usize {
    const deco = decorationGeometryForCellLayout(cell_layout);
    var count: usize = 0;
    for (cells) |cell| {
        if (classifyDecorationLead(damage, cell_grid, cell) != .draw) continue;
...
pub fn appendBackgroundDraws(allocator: std.mem.Allocator, out: *std.ArrayList(render.TextBackgroundDraw), cells: []const render.RenderableCell, cell_layout: render.CellLayout, cell_grid: render.CellGrid, damage: text_damage.NormalizedDamage) !void {
    const cell_len = count32(cells);
    var idx: u32 = 0;
    while (idx < cell_len) {
        const cell = cells[@intCast(idx)];
        const lead = classifyBackgroundLead(damage, cell_grid, cell);
        ...
        while (next_idx < cell_len) : (next_idx += 1) {
            ...
            if (classifyBackgroundNext(damage, cell_grid, row, fill_color, span_end_cell, next) != .merge) break;
        ...
        try out.append(allocator, .{
            .x_px = base_x,
            .y_px = base_y,
            .width_px = @intCast(span_cell_count * @as(u32, cell_layout.cell_w_px)),
...
pub fn appendBackgroundDrawsUnmanaged(out: *std.ArrayListUnmanaged(render.TextBackgroundDraw), cells: []const render.RenderableCell, cell_layout: render.CellLayout, cell_grid: render.CellGrid, damage: text_damage.NormalizedDamage) void {
    const cell_len = count32(cells);
    var idx: u32 = 0;
    while (idx < cell_len) {
        const cell = cells[@intCast(idx)];
...
pub fn appendBackgroundDrawCellUnmanaged(out: *std.ArrayListUnmanaged(render.TextBackgroundDraw), merge_live: *bool, merge_end_cell: *u32, cell: render.RenderableCell, cell_layout: render.CellLayout, cell_grid: render.CellGrid, damage: text_damage.NormalizedDamage) void {
    const lead = classifyBackgroundLead(damage, cell_grid, cell);
    if (lead == .skip) {
        merge_live.* = false;
        return;
    ...
    if (merge_live.* and out.items.len != 0) {
        ...
        if (sameRgba8(last.color, cell.bg) and last_row == cell_row and cell.first_cell == merge_end_cell.*) {
            last.width_px = @intCast(@as(u32, last.width_px) + @as(u32, span) * @as(u32, cell_layout.cell_w_px));
    ...
    const row: u16 = @intCast(cell.first_cell / cols);
    out.appendAssumeCapacity(.{
        .x_px = @as(i32, @intCast(col)) * @as(i32, @intCast(cell_layout.cell_w_px)),
        .y_px = @as(i32, @intCast(row)) * @as(i32, @intCast(cell_layout.cell_h_px)),
        .width_px = @intCast(@as(u32, span) * @as(u32, cell_layout.cell_w_px)),
        .height_px = cell_layout.cell_h_px,
        .color = cell.bg,
...
pub fn appendClearDraws(allocator: std.mem.Allocator, out: *std.ArrayList(render.TextClearDraw), cells: []const render.RenderableCell, cell_layout: render.CellLayout, cell_grid: render.CellGrid, damage: text_damage.NormalizedDamage) !void {
    if (damage.full) return;
    const rows = @min(cell_grid.rows, text_damage.damageRowCount(damage));
    var row: u16 = 0;
    while (row < rows) : (row += 1) {
        const dirty = text_damage.dirtyRowSpan(damage, cell_grid, row) orelse continue;
        const first_cell = dirty.firstCell(cell_grid);
        const cell_span = dirty.cellSpan();
        const span_cells = @as(u32, @max(cell_span, 1));
        try out.append(allocator, .{
            .x_px = @as(i32, @intCast(dirty.start_col)) * @as(i32, @intCast(cell_layout.cell_w_px)),
            .y_px = @as(i32, @intCast(row)) * @as(i32, @intCast(cell_layout.cell_h_px)),
            .width_px = @intCast(span_cells * @as(u32, cell_layout.cell_w_px)),
            .height_px = cell_layout.cell_h_px,
            .color = clearColorForSpan(cells, cell_grid, dirty),
...
pub fn appendClearDrawsUnmanaged(out: *std.ArrayListUnmanaged(render.TextClearDraw), cells: []const render.RenderableCell, cell_layout: render.CellLayout, cell_grid: render.CellGrid, damage: text_damage.NormalizedDamage) void {
    if (damage.full) return;
    const rows = @min(cell_grid.rows, text_damage.damageRowCount(damage));
    var row: u16 = 0;
    while (row < rows) : (row += 1) {
...
pub fn noteClearColorCell(clear_row_colors: []render.Rgba8, clear_row_matches: []bool, cell: render.RenderableCell, cell_grid: render.CellGrid, damage: text_damage.NormalizedDamage) void {
    if (damage.full) return;
    ...
    while (row < rows) : (row += 1) {
        ...
        const dirty = text_damage.dirtyRowSpan(damage, cell_grid, row) orelse continue;
        if (!text_damage.dirtySpanOverlapsCellSpan(cell_grid, dirty, cell)) continue;
        clear_row_colors[row] = .{ .r = cell.bg.r, .g = cell.bg.g, .b = cell.bg.b, .a = 255 };
        clear_row_matches[row] = true;
    }
}

pub fn appendClearRowDrawsUnmanaged(out: *std.ArrayListUnmanaged(render.TextClearDraw), clear_row_colors: []const render.Rgba8, clear_row_matches: []const bool, cell_layout: render.CellLayout, cell_grid: render.CellGrid, damage: text_damage.NormalizedDamage) void {
    if (damage.full) return;
    const rows = @min(cell_grid.rows, text_damage.damageRowCount(damage));
    var row: u16 = 0;
    while (row < rows) : (row += 1) {
...
pub fn appendCursorDraws(allocator: std.mem.Allocator, out: *std.ArrayList(render.TextCursorDraw), cursor: ?render.CursorPresentation, damage: text_damage.NormalizedDamage, cell_layout: render.CellLayout) !void {
    ...
    try out.appendSlice(allocator, cursorDrawRects(draws[0..], cursor_value, cell_layout));
...
pub fn appendCursorPrimitives(
    ...
    const cursor_value = cursor orelse return;
    try appendCursorTrailRects(allocator, cursor_trail_rects, cell_grid, cursor_value, cell_layout);
    if (!cursor_value.visible) return;
    if (classifyCursorLead(damage, cursor_value) != .draw) return;

    try appendCursorDraws(allocator, cursor_draws, cursor_value, damage, cell_layout);
    try appendCursorFillRects(allocator, cursor_fill_rects, cells, cell_grid, cursor_value, cell_layout);
    try appendCursorTextRecolorSpans(allocator, cursor_text_recolor_spans, cells, cell_grid, cursor_value);
}

fn appendCursorFillRects(allocator: std.mem.Allocator, out: *std.ArrayList(@import("draw_list.zig").CursorFillRect), cells: []const render.RenderableCell, cell_grid: render.CellGrid, cursor: anytype, cell_layout: render.CellLayout) !void {
    const first_cell: u32 = @as(u32, cursor.primary_extent.row) * @max(@as(u32, 1), @as(u32, cell_grid.cols)) + @as(u32, cursor.primary_extent.col);
    ...
    const base_x: i32 = @as(i32, @intCast(cursor.primary_extent.col)) * @as(i32, @intCast(cell_layout.cell_w_px));
    const base_y: i32 = @as(i32, @intCast(cursor.primary_extent.row)) * @as(i32, @intCast(cell_layout.cell_h_px));
    const width_px: u16 = @intCast(@as(u32, cursor.primary_extent.cols) * @as(u32, cell_layout.cell_w_px));
    const height_px: u16 = @intCast(@as(u32, cursor.primary_extent.rows) * @as(u32, cell_layout.cell_h_px));
    const geom = cursorGeometry(cursor, cell_layout);
    switch (cursor.shape) {
        .none => {},
        .block => {
...
fn appendCursorTextRecolorSpans(allocator: std.mem.Allocator, out: *std.ArrayList(@import("draw_list.zig").CursorTextRecolorSpan), cells: []const render.RenderableCell, cell_grid: render.CellGrid, cursor: anytype) !void {
...
fn appendCursorTrailRects(allocator: std.mem.Allocator, out: *std.ArrayList(@import("draw_list.zig").CursorTrailRect), cell_grid: render.CellGrid, cursor: anytype, cell_layout: render.CellLayout) !void {
    for (cursor.trail.rects[0..cursor.trail.count]) |rect| {
        try out.append(allocator, cursorTrailDrawRect(cell_grid, cursor, rect, cell_layout));
    }
}

pub fn appendCursorTrailRectsUnmanaged(out: *std.ArrayListUnmanaged(@import("draw_list.zig").CursorTrailRect), cell_grid: render.CellGrid, cursor: ?render.CursorPresentation, cell_layout: render.CellLayout) void {
    const cursor_value = cursor orelse return;
    for (cursor_value.trail.rects[0..cursor_value.trail.count]) |rect| out.appendAssumeCapacity(cursorTrailDrawRect(cell_grid, cursor_value, rect, cell_layout));
}

fn cursorTrailDrawRect(cell_grid: render.CellGrid, cursor: anytype, rect: anytype, cell_layout: render.CellLayout) @import("draw_list.zig").CursorTrailRect {
    const first_cell: u32 = if (rect.pixel_rect) pixelRectFirstCell(rect, cell_grid, cell_layout) else @as(u32, rect.extent.row) * @max(@as(u32, 1), @as(u32, cell_grid.cols)) + @as(u32, rect.extent.col);
    const x_px: i32 = if (rect.pixel_rect) rect.x_px else @as(i32, @intCast(rect.extent.col)) * @as(i32, @intCast(cell_layout.cell_w_px));
    const y_px: i32 = if (rect.pixel_rect) rect.y_px else @as(i32, @intCast(rect.extent.row)) * @as(i32, @intCast(cell_layout.cell_h_px));
    const width_px: u16 = if (rect.pixel_rect) rect.width_px else @intCast(@as(u32, rect.extent.cols) * @as(u32, cell_layout.cell_w_px));
    const height_px: u16 = if (rect.pixel_rect) rect.height_px else @intCast(@as(u32, rect.extent.rows) * @as(u32, cell_layout.cell_h_px));
...
fn pixelRectFirstCell(rect: anytype, cell_grid: render.CellGrid, cell_layout: render.CellLayout) u32 {
    std.debug.assert(rect.pixel_rect);
    std.debug.assert(cell_layout.cell_w_px != 0);
    std.debug.assert(cell_layout.cell_h_px != 0);
    const col: u32 = @intCast(@max(@divTrunc(rect.x_px, @as(i32, @intCast(cell_layout.cell_w_px))), 0));
    const row: u32 = @intCast(@max(@divTrunc(rect.y_px, @as(i32, @intCast(cell_layout.cell_h_px))), 0));
    return @min(row, @as(u32, @max(cell_grid.rows, 1)) - 1) * @max(@as(u32, cell_grid.cols), 1) + @min(col, @as(u32, @max(cell_grid.cols, 1)) - 1);
}

pub fn appendCursorDrawsUnmanaged(out: *std.ArrayListUnmanaged(render.TextCursorDraw), cursor: anytype, damage: text_damage.NormalizedDamage, cell_layout: render.CellLayout) void {
    const cursor_value = cursor orelse return;
    if (!cursor_value.visible) return;
    if (classifyCursorLead(damage, cursor_value) != .draw) return;
    if (cursor_value.shape == .none) return;
    const count_before = out.items.len;
    var draws: [4]render.TextCursorDraw = undefined;
    out.appendSliceAssumeCapacity(cursorDrawRects(draws[0..], cursor_value, cell_layout));
    assertCursorDrawCount(out.items.len - count_before, cursor_value.shape);
}

pub fn cursorDraws(allocator: std.mem.Allocator, cursor: anytype, cell_layout: render.CellLayout) ![]render.TextCursorDraw {
    ...
    _ = cursorDrawRects(draws, cursor, cell_layout);
    return draws;
}

pub fn decorationGeometryForCellLayout(cell_layout: render.CellLayout) render.DecorationGeometry {
    return decorationGeometry(cell_layout, defaultFontCellLayout(cell_layout));
}

pub fn appendRectDecorationDraws(comptime underline_color_fn: fn (render.RenderableCell) render.Rgba8, comptime text_color_fn: fn (render.RenderableCell) render.Rgba8, allocator: std.mem.Allocator, out: *std.ArrayList(render.TextDecorationDraw), cells: []const render.RenderableCell, cell_layout: render.CellLayout, cell_grid: render.CellGrid, damage: text_damage.NormalizedDamage) !void {
    const deco = decorationGeometryForCellLayout(cell_layout);
    const cols = @max(@as(u32, cell_grid.cols), 1);
    for (cells) |cell| {
        if (classifyDecorationLead(damage, cell_grid, cell) != .draw) continue;
...
pub fn appendRectDecorationCellDrawsWithLayoutUnmanaged(comptime underline_color_fn: fn (render.RenderableCell) render.Rgba8, comptime text_color_fn: fn (render.RenderableCell) render.Rgba8, out: *std.ArrayListUnmanaged(render.TextDecorationDraw), cell: render.RenderableCell, layout: RectDecorationLayout, damage: text_damage.NormalizedDamage) void {
    if (classifyDecorationLead(damage, layout.cell_grid, cell) != .draw) return;
...
fn classifyBackgroundLead(damage: text_damage.NormalizedDamage, cell_grid: render.CellGrid, cell: render.RenderableCell) BackgroundLead {
    if (cell.continuation) return .skip;
    if (!text_damage.includeSpan(damage, cell_grid, cell.first_cell, cell.cell_span)) return .skip;
    if (cell.bg.a == 0) return .transparent;
    return .span;
}

fn classifyBackgroundNext(damage: text_damage.NormalizedDamage, cell_grid: render.CellGrid, row: u16, fill_color: render.Rgba8, span_end_cell: u32, cell: render.RenderableCell) BackgroundNext {
    if (cell.continuation) return .stop_continuation;
    if (!text_damage.includeSpan(damage, cell_grid, cell.first_cell, cell.cell_span)) return .stop_damage;
    if (!sameRgba8(cell.bg, fill_color)) return .stop_color;
    if (cell.first_cell / @max(@as(u32, cell_grid.cols), 1) != row) return .stop_row;
    if (cell.first_cell != span_end_cell) return .stop_gap;
    return .merge;
}

fn clearColorForSpan(cells: []const render.RenderableCell, cell_grid: render.CellGrid, dirty: text_damage.DirtyRowSpan) render.Rgba8 {
    for (cells) |cell| {
        if (classifyClearColorCell(cell_grid, dirty, cell) != .match) continue;
...
fn classifyClearColorCell(cell_grid: render.CellGrid, dirty_span: text_damage.DirtyRowSpan, cell: render.RenderableCell) ClearColorCell {
    if (cell.continuation) return .skip;
    if (cell.bg.a != 0) return .skip;
    if (!text_damage.dirtySpanOverlapsCellSpan(cell_grid, dirty_span, cell)) return .skip;
    return .match;
}

fn classifyDecorationLead(damage: text_damage.NormalizedDamage, cell_grid: render.CellGrid, cell: render.RenderableCell) DecorationLead {
    if (cell.continuation) return .skip;
...
fn cursorDrawRects(out: []render.TextCursorDraw, cursor: anytype, cell_layout: render.CellLayout) []const render.TextCursorDraw {
    const count = cursorDrawCountExact(cursor.shape);
    ...
    switch (cursor.shape) {
        .block => out[0] = .{ .x_px = base_x, .y_px = base_y, .width_px = cell_layout.cell_w_px, .height_px = cell_layout.cell_h_px, .color = cursor_color },
        .beam => out[0] = .{ .x_px = base_x, .y_px = base_y, .width_px = geom.beam_w_px, .height_px = cell_layout.cell_h_px, .color = cursor_color },
        .underline => out[0] = .{ .x_px = base_x, .y_px = base_y + @as(i32, @intCast(cell_layout.cell_h_px - geom.underline_h_px)), .width_px = cell_layout.cell_w_px, .height_px = geom.underline_h_px, .color = cursor_color },
        .hollow => {
            const stroke = geom.hollow_stroke_px;
            out[0] = .{ .x_px = base_x, .y_px = base_y, .width_px = cell_layout.cell_w_px, .height_px = stroke, .color = cursor_color };
            out[1] = .{ .x_px = base_x, .y_px = base_y + @as(i32, @intCast(cell_layout.cell_h_px - stroke)), .width_px = cell_layout.cell_w_px, .height_px = stroke, .color = cursor_color };
            out[2] = .{ .x_px = base_x, .y_px = base_y, .width_px = stroke, .height_px = cell_layout.cell_h_px, .color = cursor_color };
            out[3] = .{ .x_px = base_x + @as(i32, @intCast(cell_layout.cell_w_px - stroke)), .y_px = base_y, .width_px = stroke, .height_px = cell_layout.cell_h_px, .color = cursor_color };
        },
        .none => unreachable,
    }
...
fn defaultFontCellLayout(cell_layout: render.CellLayout) render.FontCellLayout {
    const thickness: f32 = @floatFromInt(scaledDecorationThickness(cell_layout.cell_h_px));
    const baseline: f32 = @floatFromInt(cell_layout.baseline_px);
    return .{
        .ascent_px = baseline,
        .descent_px = @floatFromInt(@as(i32, cell_layout.cell_h_px) - @as(i32, cell_layout.baseline_px)),
        .line_gap_px = 0,
...
fn decorationGeometry(cell_layout: render.CellLayout, font_cell_layout: render.FontCellLayout) render.DecorationGeometry {
    return .{
        .underline_y_px = std.math.clamp(@as(i32, @intFromFloat(@round(font_cell_layout.underline_pos_px))), 0, @as(i32, @intCast(cell_layout.cell_h_px - 1))),
        .underline_h_px = @max(@as(u16, @intFromFloat(@round(font_cell_layout.underline_thickness_px))), 1),
        .strikethrough_y_px = std.math.clamp(@as(i32, @intFromFloat(@round(font_cell_layout.strikethrough_pos_px))), 0, @as(i32, @intCast(cell_layout.cell_h_px - 1))),
        .strikethrough_h_px = @max(@as(u16, @intFromFloat(@round(font_cell_layout.strikethrough_thickness_px))), 1),
    };
}

fn cursorGeometry(cursor: anytype, cell_layout: render.CellLayout) render.CursorGeometry {
    return .{
        .beam_w_px = configuredThicknessPx(cell_layout.cell_w_px, cursor.beam_thickness),
        .underline_h_px = configuredThicknessPx(cell_layout.cell_h_px, cursor.underline_thickness),
        .hollow_stroke_px = 2,
    };
}
...
test "configured beam and underline thickness affect cursor geometry" {
    ...
    const beam_draws = try cursorDraws(std.testing.allocator, beam, cell_layout);
    ...
    try appendCursorFillRects(std.testing.allocator, &list, &.{}, .{ .cols = 1, .rows = 1 }, underline, cell_layout);
...
test "visible no-shape produces no cursor draws fill or recolor" {
    ...
    const draws = try cursorDraws(std.testing.allocator, cursor, cell_layout);
    ...
    try appendCursorPrimitives(std.testing.allocator, &draw_list, &fill_list, &recolor_list, &trail_list, &.{}, .{ .cols = 1, .rows = 1 }, cursor, damage, cell_layout);
