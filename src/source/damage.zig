...
pub fn cursorPresentationChanged(prior: source_vt.PublicationSource, current: source_vt.PublicationSource) bool {
    ...
    if ((prior.cursor.blink or current.cursor.blink) and
        prior.cursor_phase_visible != current.cursor_phase_visible) return true;
    return false;
}
...
test "source damage preserves dirty row spans and sentinels while canonicalizing" {
    const dirty_rows = [_]u8{ 1, 1, 0 };
    var dirty_cols_start = [_]u16{ 1, 3, 2 };
    var dirty_cols_end = [_]u16{ 2, 0, 1 };
    ...
    try std.testing.expectEqual(@as(u16, 0), dirty_cols_end[1]);
